...
def analyze():
	...
	result=sv.predict(ex1)
	class2=croprecom(result)
	Y_pred_sv =sv.predict(X_test1)
	Y_pred_sv.shape
	score_sv = round(accuracy_score(Y_pred_sv,y_test1)*100,2)
	...
	result=rf.predict(ex1)
	class5=croprecom(result)
	Y_pred_rt =rf.predict(X_test1)
	Y_pred_rt.shape
	score_rf= round(accuracy_score(Y_pred_rt,y_test1)*100,2)
            
		
	result=model.predict(ex1)
	class6=croprecom(result)
	...
	return render_template('index.html',region=region,area=area,N=N,P=P,k=k,ph=ph,temp=temp,humidity=humidity,rain=rain,class1=class1,score_nb=score_nb,class2=class2,score_sv=score_sv,class3=class3,score_knn=score_knn,class4=class4,score_dt=score_dt,class5=class5,score_rf=score_rf,class6=class6,score_nn=score_nn)
